...
function isValidCustomPreset(p) {
  if (!p || typeof p !== "object") return false;
  const x = p;
  return typeof x.id === "string" && x.id.length > 0 && x.id.length <= 128 && typeof x.label === "string" && x.label.length > 0 && x.label.length <= 128 && typeof x.hint === "string" && x.hint.length <= 128 && VALID_ALGORITHMS.includes(x.algorithm) && typeof x.threshold === "number" && Number.isFinite(x.threshold) && typeof x.spread === "number" && Number.isFinite(x.spread) && typeof x.brightness === "number" && Number.isFinite(x.brightness) && typeof x.contrast === "number" && Number.isFinite(x.contrast) && typeof x.resizePercent === "number" && Number.isFinite(x.resizePercent);
}
...
var ImageDitherPlugin = class extends import_obsidian.Plugin {
  ...
  async loadSettings() {
    ...
    if (!Array.isArray(this.settings.customPresets)) {
      ...
      this.settings.customPresets = this.settings.customPresets.filter(isValidCustomPreset);
    }
  }
  async saveSettings() {
...
var ImageDitherSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  renderCustomPresetsSetting(containerEl) {
    ...
    new import_obsidian.Setting(containerEl).setName("Custom presets").setHeading();
    customPresets.forEach((preset) => {
      const descFrag = document.createDocumentFragment();
      descFrag.append(
        `${preset.algorithm} \xB7 threshold ${preset.threshold} \xB7 sharpness ${preset.spread}% \xB7 brightness ${preset.brightness > 0 ? "+" : ""}${preset.brightness}% \xB7 contrast ${preset.contrast > 0 ? "+" : ""}${preset.contrast}% \xB7 resize ${preset.resizePercent}%`
      );
      new import_obsidian.Setting(containerEl).setName(preset.label).setDesc(descFrag).addButton((btn) => {
...
function sanitizeFileBaseName(name) {
  const cleaned = name.replace(/[\x00-\x1f\x7f]/g, "").replace(/[\\/:*?"<>|]/g, "-").replace(/\s+/g, " ").trim().replace(/\.+$/g, "");
